...
function ECHO_FUNCTION {
	...
	print "\r[0K  Audirvana Origin: $1\n  Last.fm: $SCROBBLE_MESSAGE" >> ./logs/shell_audirvana-origin-scrobbler.log.log
	tput cup 4
}
...
while true; do
	...
	print "休眠开始时长：$SLEEP_TIME，空闲时间：$AUDIRVANA_IDLE_TIME,空闲阈值：$AUDIRVANA_IDLE_THRESHOLD，长：$LONG_SLEEP_TIME" >> ./logs/shell_audirvana-origin-scrobbler.log.log
	sleep "$SLEEP_TIME"
done
